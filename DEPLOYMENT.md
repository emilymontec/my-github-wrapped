...
5. Opcional pero recomendado: **cuenta en [Mailgun](https://www.mailgun.com)** (emails transaccionales, requiere verificar un dominio de envío) y **API key de [Hugging Face](https://huggingface.co/settings/tokens)** (narración de insights con IA). El proyecto funciona sin ninguna de las dos — cae a plantillas/no-op documentados — pero con ellas la experiencia es mejor.
...
| `HUGGINGFACE_API_KEY` | Opcional — [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) |
| `HUGGINGFACE_MODEL` | Opcional — por defecto `mistralai/Mistral-7B-Instruct-v0.3`; cambiar si ese modelo deja de estar disponible en el free tier |
| `MAILGUN_API_KEY` / `MAILGUN_DOMAIN` / `MAILGUN_FROM_EMAIL` | Opcional — dashboard de Mailgun; el dominio debe estar verificado (registros DNS) antes de poder enviar |
| `MAILGUN_BASE_URL` | Solo si tu cuenta de Mailgun es de la región EU (`https://api.eu.mailgun.net`) |
